...
import os
import optuna
from kevin_toolbox.patches import for_optuna, utils
...
def test_sample_from_feasible_domain():
    ...
    assert isinstance(utils.get_value_by_name(var=res, name=":for_ema:keep_ratio"), (float,))
    assert isinstance(utils.get_value_by_name(var=res, name=":iou_thr_ls@0"), (float,))
    assert isinstance(utils.get_value_by_name(var=res, name=":iou_thr_ls@1"), (int,))
