...
转换为 torch.tensor
"""
inputs_ls.append([torch.as_tensor([i] * 4, dtype=torch.float32) for i in inputs])
outputs_raw_ls.append([torch.as_tensor([i] * 4, dtype=torch.float32) for i in outputs_raw])
outputs_unbiased_ls.append([torch.as_tensor([i] * 4, dtype=torch.float32) for i in outputs_unbiased])
