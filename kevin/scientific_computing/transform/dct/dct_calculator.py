...
class DCT_Calculator:
    """
        ...
        ps：本模块计算DCT时并没有使用类似FFT的动态规划方式来节省计算量，因为本模块更多地关注使用gpu并行计算的场景，而
    ...
    def __init__(self, **kwargs):
        ...
        # 变量
        self.trans_matrix_repos = dict()  # {sampling_points_num/r_num: trans_matrix, ...}
    ...
    def cal(self, **kwargs):
        ...
        if not torch.is_tensor(paras["x"]):
            paras["x"] = torch.tensor(paras["x"], dtype=torch.float32, device=paras["device"])
        elif paras["x"].device != paras["device"]:
            paras["x"] = paras["x"].to(paras["device"])
        x = paras["x"]
        #
        ...
        for i, (r_num, c_num) in enumerate(
            ...
            # 获取转换矩阵
            if r_num not in self.trans_matrix_repos or self.trans_matrix_repos[r_num].shape[-1] < c_num:
                # 新增到库
                self.trans_matrix_repos[r_num] = torch.tensor(dct.generate_trans_matrix(shape=[r_num, r_num]),
                                                              dtype=torch.float32, device=paras["device"])
            B = self.trans_matrix_repos[r_num][..., :c_num]
            # 将待处理维度放到最后
            x = x.transpose(-1 - i, -1)
            # 变换
